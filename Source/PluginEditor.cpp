...

{
    ...
    auto getParamHelper = [&params, &apv](const auto& name) -> auto&
        {
            return getParam(apv, params, name);
        };
    ...
    auto makeAttachmentHelper = [&params, &apv](auto& attachment, const auto& name, auto& slider)
        {
            makeAttachment(attachment, apv, params, name, slider);
        };
